...
async def update():
    ...
    rules = [
        ...
        {"id": "core_4", "text": "FILTRO INTELIGENTE (LOOK VS DO): Excluir absoluta y automáticamente todo procedimiento de rutina de enfermería (ej. signos vitales, control, valoración) y fluidos basales o hidratación inespecífica (ej. dextrosa, solución fisiológica, ringer, PHP) de CUALQUIER sección. Son irrelevantes a menos que marquen un evento crítico.", "priority": "alta", "active": True, "processed": True},
    ]
    await db.golden_rules.update_one(
        {"_id": "core"},
        {"$set": {"rules": rules, "updated_at": datetime.now(timezone.utc).isoformat(), "updated_by": "sistema"}}
    )
    print("Core rule added to DB.")
