...
_embeddings = None  # LlamaIndex GeminiEmbedding
...
def get_embeddings():
    """Get or create Gemini embeddings model (text-embedding-004 via LlamaIndex)."""
    global _embeddings
    if _embeddings is None:
        try:
            from llama_index.embeddings.gemini import GeminiEmbedding
            _embeddings = GeminiEmbedding(
                model_name="models/text-embedding-004",
                api_key=settings.GEMINI_API_KEY,
            )
            log.info("[Qdrant] Embeddings model initialized (Gemini text-embedding-004)")
        except Exception as e:
...
async def store_feedback_vector(
    ...
    try:
        ...
        # Create embedding
        vector = embeddings.get_text_embedding(feedback_text)
...
async def search_similar_feedback(
    ...
    try:
        ...
        query_vector = embeddings.get_text_embedding(query_text)
