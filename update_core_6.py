...
async def update():
    ...
    rules = [
        ...
        {"id": "core_10", "text": "10. INTEGRIDAD DE EXTRACCIÓN (ANTI-TRUNCAMIENTO): Al extraer o listar textos médicos (procedimientos, diagnósticos, etc.), NUNCA trunques los nombres. Extrae la frase anatómica o técnica completa. Está terminantemente prohibido generar frases terminadas en preposiciones cortadas (ej. descarta un 'Cirugía en' o 'Tratamiento de'). Si un texto en el documento original está ilegiblemente cortado de ese modo, omítelo completamente.", "priority": "alta", "active": True, "processed": True},
    ]
    await db.golden_rules.update_one(
        {"_id": "core"},
        {"$set": {"rules": rules, "updated_at": datetime.now(timezone.utc).isoformat(), "updated_by": "sistema"}}
    )
    print("10 Core rules optimized in DB.")
